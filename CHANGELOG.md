...
- **WakeLock refresh loop**: New coroutine re-acquires WakeLock every 25 minutes before Samsung's 30-minute timeout hits, preventing system throttling during long-running proxy sessions.
- **Sticky restart handling**: Service now uses `START_REDELIVER_INTENT` so the last configuration Intent is replayed automatically after system-initiated restarts instead of starting with a null intent.
...
### Internal
- Bumped versionCode: 5 → 6, versionName: 1.4.0 → 1.5.0
